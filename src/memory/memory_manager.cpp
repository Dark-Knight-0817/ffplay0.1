...
bool MemoryManager::initializeComponents() {
    try {
        ...
        if (config_.use_frame_allocator) {
            FrameAllocator::Config frame_config;
...
MemoryManager::GlobalStatistics MemoryManager::getGlobalStatistics() const {
    ...
    // 计算总体统计
    stats.total_memory_usage = stats.pool_stats.current_usage +
                               stats.tracker_stats.current_usage +
                               stats.frame_stats.total_memory_usage +
                               stats.packet_stats.current_in_use;

    stats.total_objects = stats.pool_stats.allocation_count +
                          stats.tracker_stats.allocation_count +
                          stats.frame_stats.total_allocated +
                          stats.packet_stats.total_created;
...
void MemoryManager::optimizeConfiguration() {
    ...
    double current_usage_ratio = static_cast<double>(stats.total_memory_usage) / config_.max_total_memory;
...
std::string MemoryManager::generateComprehensiveReport() const {
    ...
    oss << "Total Memory Usage: " << stats.total_memory_usage << " bytes\n";
    oss << "Total Objects: " << stats.total_objects << "\n";
...
void MemoryManager::checkMemoryPressure() {
    auto stats = getGlobalStatistics();
    double usage_ratio = static_cast<double>(stats.total_memory_usage) / config_.max_total_memory;
...
void MemoryManager::handleMemoryPressure(PressureLevel level) {
    ...
    if (pressure_callback_) {
        ...
        PressureEvent event(level, stats.total_memory_usage, config_.max_total_memory, description);
        pressure_callback_(event);
    }
}
...
void MemoryManager::collectGlobalStatistics() {
    ...
    global_stats_ = getGlobalStatistics();
...
void MemoryManager::recordMemoryUsage() {
    auto now = std::chrono::steady_clock::now();
    memory_history_.emplace_back(now, global_stats_.total_memory_usage);
...
template CacheManager<int, std::vector<uint8_t>>& MemoryManager::getCacheManager<int, std::vector<uint8_t>>();
